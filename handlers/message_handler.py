...
async def handle_default_ai(message: Message, state: FSMContext):
    ...
    if is_user_banned(user_id):
        await message.answer("⛔ Вы забанены в этом боте. Обратитесь к администратору.")
        return
    ...
    try:
        if ai_name == "groq":
            ...
            answer = ask_openai_with_history(message.text, history)
